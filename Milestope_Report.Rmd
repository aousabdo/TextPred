...
blogtxt <- readLines(en_US_blog, 100000)
twittertxt <- readLines(en_US_twitter, 100000)
newstxt <- readLines(en_US_news, 100000)
...
Buile corpus from the merge text file above: 

```{r, cache=TRUE}
corpus <- VCorpus(VectorSource(txt))
```

Clean corpus using `ITSA` and `tm` packages:

```{r}
corpus <- ITSA::cleanCorpus(corpus,removeNum = TRUE, removeSpecialChar = TRUE, stemCorpus = TRUE)
```